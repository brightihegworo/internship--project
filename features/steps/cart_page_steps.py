...
def verify_cart_count(context, expected_count):
    ...
    assert expected_count == actual_text, f'Expected {expected_count} but got actual {actual_count}'


@then('Verify cart has correct product and price')
def verify_product_name(context):
    actual_name = context.driver.find_element(*PRODUCT_NAME).text
    assert context.product_name[:30] in actual_name, f'Expected {context.product_name} but got actual {actual_name}'

    actual_price = context.driver.find_element(*PRODUCT_PRICE).text
    assert context.product_price == actual_price, f'Expected {context.product_price} but got actual {actual_price}'
